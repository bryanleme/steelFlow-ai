...
| R-006 | Cenários OOD receberem recomendação indevida. | Baixa | Crítico | Guard de envelope condicional, distância histórica, restrições duras e recusa explícita; 3/3 sondas foram bloqueadas. | Mitigado no pipeline; validar novamente na interface |
...
| R-013 | Proxy de TBH ser confundida com o modelo TBH oficial ou com efeito causal. | Média | Alto | Nome, fórmula, janela de calibração e ausência de avaliação final aparecem em cenário, manifest e relatório. | Mitigado por contrato; revisar na Fase 7 |
...
| R-015 | Risco de parada parecer responsivo a controles que o modelo não recebe. | Média | Alto | Campo `scenario_invariant`, motivo explícito e documentação do objetivo contextual. | Mitigado por contrato; revisar linguagem da interface |
